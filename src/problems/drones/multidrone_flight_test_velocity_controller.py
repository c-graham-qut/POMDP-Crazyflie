...
def set_variables(): #sets all important variables here to avoid having the main program overflow with variable definitions
    ...
    velocity_delta = 0.1 # [m/s] defines the speed for the velocity controller
    critical_dist = 0.2
    wait_time = 0.15
    ...
    return break_program, come_home, hover_height, MIN_DISTANCE, MAX_HEIGHT, grid_distance, velocity_delta, critical_dist, wait_time, \
...
def calc_command(hover_height, grid_distance, old_x_com, old_y_com, NumberOfLines, airborne, swarm_status, meas_positions, multiranger_meas, obstacles, counter): #calculates next position for the drones depending on POMDP output
  ...
  if NewNumberOfLines != NumberOfLines: # i.e. a new line (= new command) has been added
    ...
    # the next commands are from the POMDP model, so if they are changed there they also have to be changed here
      ...
      x_com = velocity_delta
      y_com = velocity_delta

    elif command == "WIDER":
      #positive in y-direction means it goes left (from the drones initial point of view)
      y_com = [velocity_delta, velocity_delta, 0, - velocity_delta, - velocity_delta] # if the NumberOfDrones changes this vector has to be changed as well
      print('\n\nAction WIDER chosen\n\n')

    elif command == "NARROWER":
      #positive in y-direction means it goes left (from the drones initial point of view)
      y_com = [-velocity_delta, -velocity_delta, 0, velocity_delta, velocity_delta] # if the NumberOfDrones changes this vector has to be changed as wells
      print('\n\nAction NARROWER chosen\n\n')

    elif command == "REARRANGE":
      # comment out all the code for this case (so the action REARRANGE does not do anything) if it does not work

      # all drones that are behind the drone with the lowest row number move forward. The drone(s) with the lowest row number stay at their current position
      min_row = np.amin(gridpositions[:,0])
      for drone in range(NumberOfDrones):
        if gridpositions[drone,0] > min_row:
          x_com[drone] = velocity_delta
        else:
          x_com[drone] = 0
  ...
  # if the new command equals the old command (same action chosen)
  if x_com == old_x_com and y_com == old_y_com:
    counter = counter + 1
  if counter > 10:
    pos[0,0] = 0 # Because it is Python something has to be done, but basically the whole matrix stays 0 (= no movement / HOVER)
  else:
    pos[:,0] = x_com
    pos[:,1] = y_com
    pos[:,2] = 0
  ...
  return pos, NewNumberOfLines, takeoff, command_land, counter
...
def Changes2File(gridpositions, multiranger_meas, grid_distance, swarm_status, simulation_step, obstacles):
  ...
  for drone in swarm_status:
    ...
    if drone > 0 and multiranger_meas[drone - 1, 0] < dist * grid_distance and \
     inMap([gridpositions[drone-1,0]-round2grid(multiranger_meas[drone - 1, 0], grid_distance) + 1, gridpositions[drone-1,1]]):
        i += 1
        ...
        % (gridpositions[drone-1,0]-round2grid(multiranger_meas[drone - 1, 0], grid_distance) + 1, gridpositions[drone-1,1], \
         gridpositions[drone-1,0]-round2grid(multiranger_meas[drone - 1, 0], grid_distance) + 1, gridpositions[drone-1,1])
...
def send_swarm_velocity(scf, velocities): 
  cf = scf.cf
  if Fly_Go:
    # print('FlyGo = True')
    send_velocity_world_setpoint(velocities[0], velocities[1], velocities[2], velocities[3])
  ...
  with Swarm(uris, factory=factory) as swarm:
    ...
    with keyboard.Listener(on_press=on_press) as listener: 
      ...
      while a.value == 0 and break_program == False and come_home == False: 
        ...
        com_positions, NumberOfLines, takeoff, land_command, counter = calc_command(hover_height, grid_distance, x_com, y_com, NumberOfLines, airborne, swarm_status, meas_positions, multiranger_meas, obstacles, counter)
        ...
        if takeoff == False:
          # sends to the commanded velocities to the drones. Here the input argument for the drones is VELOCITIES which connects the URI to the 
          # commanded velocities for that drones.
          ...
          swarm.parallel(send_swarm_velocity, args_dict = VELOCITIES)
        else: 
          n.value = 1
...
if __name__ == '__main__': #basically handles the two parallel processes which invoke drone_control() and POMDP()
    ...
    break_program, come_home, hover_height, MIN_DISTANCE, MAX_HEIGHT, grid_distance, velocity_delta, critical_dist, wait_time, \
