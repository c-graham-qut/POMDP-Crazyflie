...
def calc_command(hover_height, grid_distance, x_com, y_com, NumberOfLines, airborne, swarm_status, meas_positions, multiranger_meas, obstacles, waiting): #calculates next position for the drones depending on POMDP output
  ...
  if waiting > 15:
    # after 15*0.1 seconds the distances around the drones are measured
  ...
  return pos, NewNumberOfLines, takeoff, command_land, waiting
...
def Changes2File(gridpositions, multiranger_meas, grid_distance, swarm_status, simulation_step, obstacles):
  ...
  dist = 5 # maximum number of grid fields to detect obstacles (obstacles further away will not be detected)
  ...
  for drone in swarm_status:
    obs_pos = [gridpositions[drone-1,0]-round2grid(multiranger_meas[drone - 1, 0], grid_distance), gridpositions[drone-1,1]]
    ...
    if drone > 0 and multiranger_meas[drone - 1, 0] < dist * grid_distance and obs_pos not in obstacles and inMap(obs_pos):
      ...
      # obstacle in front
      second_string += "Add Obstacles: (%d, %d) (%d, %d)\n" % (obs_pos[0], obs_pos[1], obs_pos[0]+1, obs_pos[1])
    ...
    obs_pos = [gridpositions[drone-1,0], gridpositions[drone-1,1]-round2grid(multiranger_meas[drone - 1, 1], grid_distance)]
    if drone > 0 and multiranger_meas[drone - 1, 1] < dist * grid_distance and obs_pos not in obstacles and inMap(obs_pos):   
        i += 1
        # obstacle to the left of the drones (multiranger_meas[drone - 1, 1] = multiranger.left)
        second_string += "Add Obstacles: (%d, %d) (%d, %d)\n" % (obs_pos[0], obs_pos[1], obs_pos[0], obs_pos[1])
    ...
    obs_pos = [gridpositions[drone-1,0], gridpositions[drone-1,1]+round2grid(multiranger_meas[drone - 1, 3], grid_distance)]
    if drone > 0 and multiranger_meas[drone - 1, 1] < dist * grid_distance and obs_pos not in obstacles and inMap(obs_pos):   
        i += 1
  ...
  if i > 0: # only wirte to file if there are changes
    input_file = open("../../../problems/drones/changes/multiranger-changes.txt", "w") # append to file
  ...
  with Swarm(uris, factory=factory) as swarm:
    ...
    with keyboard.Listener(on_press=on_press) as listener: 
      ...
      while a.value == 0 and break_program == False and come_home == False: 
        ...
        com_positions, NumberOfLines, takeoff, land_command, waiting = calc_command(hover_height, grid_distance, x_com, y_com, NumberOfLines, airborne, swarm_status, meas_positions, multiranger_meas, obstacles, waiting)
